...
#include "maidsafe/routing/test_utils.h"
...
using namespace maidsafe::routing::test;
...
GenericNode::GenericNode(const passport::Pmid& pmid, bool has_symmetric_nat)
	    ...
	    endpoint_.port(maidsafe::test::GetRandomPort());
		InitialiseFunctors();
		routing_.reset(new Routing(pmid, new top::storage::TopStorageMessageHandler(*this)));
		std::lock_guard<std::mutex> lock(mutex_);
		id_ = next_node_id_++;

}
GenericNode::GenericNode(const passport::Pmid& pmid, uint16_t local_port, bool has_symmetric_nat)
        : functors_(),
        id_(0),
        node_info_plus_(std::make_shared<NodeInfoAndPrivateKey>(MakeNodeInfoAndKeysWithFob(pmid))),
        maid_(),
...
void GenericNode::SendDirect(const maidsafe::NodeId& destination_id,
        RoutingMessage& message, bool cacheable,
        maidsafe::routing::ResponseFunctor response_functor) {
    routing_->SendDirect(destination_id, message, cacheable, response_functor);
}
void GenericNode::SendGroup(const maidsafe::NodeId& destination_id,
        RoutingMessage& message, bool cacheable,
        maidsafe::routing::ResponseFunctor response_functor) {
    routing_->SendGroup(destination_id, message, cacheable, response_functor);
}

void GenericNode::SendMessage(const NodeId& destination_id, RoutingMessage& proto_message) {
#ifdef _WIN32
    routing_->pimpl_->WinSendMessage(destination_id, proto_message);
#else
...
void GenericNode::RudpSend(const NodeId& peer_node_id, const RoutingMessage& message,
    rudp::MessageSentFunctor message_sent_functor) {
    routing_->pimpl_->network_->RudpSend(peer_node_id, message, message_sent_functor);
}

void GenericNode::SendToClosestNode(const RoutingMessage& message) {
...
void GenericNode::get_routingtable_endpoint(std::vector<std::string> &rtvec){
    routing_->pimpl_->get_routingtable_endpoint(rtvec);
}
...
void GenericNetwork::SetUp() {
    ...
    EXPECT_EQ(ReturnCode::kSuccess, f2.get());
    EXPECT_EQ(ReturnCode::kSuccess, f1.get());
}
